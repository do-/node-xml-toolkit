...
const XMLSchemata = class extends Map {
	...
	addSchema (node, options) {
		...
		const schema = new XMLSchema (this, targetNamespace, node); if (targetNamespace === XMLSchema.namespaceURI) schema.getSimpleTypeClass = node => XSSimpleType.forName (node.attributes.name)
