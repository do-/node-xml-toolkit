...
const XMLSchemata = class extends Map {
	...
	getType (ref) {
		...
		if (namespaceURI === XMLSchemaNamespace) return {
	...
	getSchemaByLocalName (localName) {
		...
		if (!ns) throw new Error ('Unknown name: ' + localName + ', known are: ' + [...this [IDX].entries ()])
	...
	async addSchema (node, options = {}) {
		...
		const imp = node.children.filter (e => e.localName === 'import' && e.namespaceURI === XMLSchemaNamespace)
	...
	async addFile (fn, options = {}) {
		...
		for await (const node of 
			...
			new XMLReader ({
				filterElements: e => e.namespaceURI === XMLSchemaNamespace,
				map: adjustNode,
			})
...
console.log ([node.localName, node.namespaceURI])
		if (node.localName !== 'schema' || node.namespaceURI !== XMLSchemaNamespace) {
		...
		const {addLocation} = options; for (const {localName, namespaceURI, attributes} of node.children)
			...
			if (localName === 'import' && namespaceURI === XMLSchemaNamespace)
