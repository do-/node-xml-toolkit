...
class XSSimpleType {
	...
	* strings (value) {
	...
	testBounds (value) {
		...
		if (this.minInclusive != null && (ordered   = this.toOrdered (value)) <  this.minInclusive) return (`The value '${value}' is less than allowed '${this.minInclusive}'`)

		if (this.maxInclusive != null && (ordered ??= this.toOrdered (value)) >  this.maxInclusive) return (`The value '${value}' is greater than allowed '${this.maxInclusive}'`)
			
		if (this.minExclusive != null && (ordered ??= this.toOrdered (value)) <= this.minExclusive) return (`The value '${value}' is less or equal than the threshold '${this.minExclusive}'`)

		if (this.maxExclusive != null && (ordered ??= this.toOrdered (value)) >= this.maxExclusive) return (`The value '${value}' is greater or equal than the threshold '${this.maxExclusive}'`)
	...
	testLength (value) {
		...
		if (length === null) {

			{
				...
				const {maxLength} = this; if (l > maxLength) return ['XVS-00004', value, l, units, maxLength]

			}

			{

				const {minLength} = this; if (l < minLength) return ['XVS-00005', value, l, units, minLength]
			...
			if (l !== length) return ['XVS-00006', value, l, this.units, length]
