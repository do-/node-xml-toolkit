...
class DT7 {
	...
	constructor (src) {
		...
		const {length} = src; if (length < MIN_LENGTH) die (['XVS-00005', this.src, length, MIN_LENGTH])
	...
	parse () {
		...
		if (this.src.charCodeAt (this.posDay) !== CH_MINUS) die (['XVS-00027', this.src, this.posDay, this.src.charAt (this.posDay)])
		
		const {length} = this.src

		if (length < this.endOfDate) die (['XVS-00028', this.src, this.day])
	...
	parseAfterDate () {
		...
		for (const pos of [this.endOfHour, this.endOfMinute]) if (this.src.charCodeAt (pos) !== CH_COLON) die (['XVS-00029', this.src, pos, this.src.charAt (pos)])
	...
	validateYear () {
		...
		if (year.charCodeAt (base) === CH_0 && length - base !== 4) die (['XVS-00026', this.src, year])
		
		for (let i = base; i < length; i ++) {

			const c = year.charCodeAt (i)

			if (c > CH_9 || c < CH_0) die (['XVS-00030', this.src, year, year.charAt (i), i])
	...
	validateMonth () {
		...
		if (month < '01' || month > '12') die (['XVS-00031', this.src, month])
	...
	validateDay () {
		...
		if (day < '01' || day > '31') die (['XVS-00032', this.src, day])

		if (day === '31' || (this.month === '02' && day > '28')) {

			if (new Date (this.src.substring (0, this.endOfDate)).getDate () != day) die (['XVS-00033', this.src])

		}

	}

	validate2digits (value, code, max = '59') {

		if (value < '00' || value > max) die ([code, this.src, value])
	...
	validateTZ () {
		...
		switch (tz.charCodeAt (0)) {
			...
			case CH_MINUS:
				break
			default:
				die (['XVS-00039', this.src, tz.charAt (0)])
		}

		if (tz.length !== 6) die (['XVS-00040', this.src, tz.length])

		if (tz.charCodeAt (3) !== CH_COLON) die (['XVS-00041', this.src])
