		...
		for (const node of nodes) {
			...
			switch (type) {
				...
				case SAXEvent.TYPES.END_ELEMENT:
					...
					if (this.element === null) throw new Error (`${nodes.linePos}Unbalanced end element`)
					if (this.element.name != node.name) throw new Error (`${nodes.linePos}Unmatched end element, </${this.element.name}> expected`)
