		...
		switch (localName) {
			...
			case 'attribute':
				...
				if (type) {
					...
					this.buf += this.getScalar (this.xs.getType (type), v)

				}
				else {

					for (const child of node.children) if (child.localName === 'simpleType') this.buf += this.getScalar (child, v)
	...
	appendContent (node, data) {
		...
		switch (localName) {
			...
			case 'simpleType':
				...
				if (data != null) this.buf += this.getScalar (node, data)
