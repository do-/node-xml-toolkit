...
*/
	...
	appendContent (node, data) {
		...
		const {localName, namespaceURI, attributes, children, targetNamespace} = node, {name, type} = attributes
...
//					if (complexType) this._aComplexType (buf, complexType, content)
					...
					if (type) {
						...
						this.appendContent (this.xs.getByReference (type), d)
					
					}
					else {
					
						for (const i of children) this.appendContent (i, d)
...
console.log ({v, type, restriction})
