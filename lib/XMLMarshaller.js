...
const XMLMarshaller = class {
	...
	appendStartTag (node, data, qName) {
		...
		if (data [Symbol.for ('type')]) {

			let {targetNamespace} = this.xs.getSchemaByLocalName(data [Symbol.for ('type')])

			this.printer
				.writeAttribute ('xmlns:xsi', 'http://www.w3.org/2001/XMLSchema-instance')
				.writeAttribute ('xsi:type', this.ns.QName (data [Symbol.for ('type')], targetNamespace))
		}
	...
	appendElementBody (node, data) {
		...
		if (data [Symbol.for ('type')]) {
			...
			this.appendElementBody (this.schema.get ('getOrganizationsByEasRequest'), d)
		}
