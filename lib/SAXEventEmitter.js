	...
	publishMarkup (chunk) {
		...
		switch (chunk.charCodeAt (1)) {
			...
			case CH_SLASH:
				...
				return this.emit ('EndElement', chunk)

			default:
				
				const isSelfEnclosing = chunk.charCodeAt (chunk.length - 2) === CH_SLASH
				
				if (!isSelfEnclosing) this.level ++

				this.emit ('StartElement', chunk)

				if (isSelfEnclosing) this.emit ('EndElement', chunk)
