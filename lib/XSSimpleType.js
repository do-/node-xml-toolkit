...
class XSSimpleType {
	...
	* strings (value) {
	...
	test (s) {
		...
		return this.testBounds (s) ?? this.testPatterns (s) ?? (super.test ? super.test (s) : null)

	}	

	testBounds (s) {

		if (s.length > this.maxLength) return `The value '${s}' is '${s.length}' chars long, which exceeds the allowed maximum of ${this.maxLength}`
