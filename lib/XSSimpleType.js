...
class XSSimpleType {
	...
	* strings (value) {
	...
	testLength (s) {
		...
		if (length > this.maxLength) return `The value '${s}' is ${s.length} chars long, which exceeds the allowed maximum of ${this.maxLength}`

		if (length < this.minLength) return `The value '${s}' is ${s.length} chars long, which is less than the allowed minimum of ${this.minLength}`
