...
const XMLPrinter = class {
	...
	openElement (name) {
		...
		if (this.stack.length !== 0) {
			if (this.attrSpace && this.lastCharCode === CH_QQ) this.text += this.newLine (-1)
	...
	writeAttribute (name, value) {
		...
		this.text += `${this.attrSpace ? `${this.newLine (-1)}${this.attrSpace}` : ' '}${name}="${this.ESC_ATTR.escape (value)}"`
	...
	closeElement () {
		...
		if (this.isOpening) {
			if (this.attrSpace && this.lastCharCode === CH_QQ) this.text += this.newLine ()
			this.text += ` />`
			this.isOpening = false
		}
		else {
			if (this.space && this.lastCharCode === CH_GT) this.text += this.newLine ()
			this.text += `</${name}>`
		}
	...
	writeXMLDecl (encoding, standalone) {
		...
		if (standalone != null) {
			...
			this.text += ` standalone="${standalone}"`
