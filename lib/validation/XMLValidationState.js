...
class XMLValidationState {
	...
	constructor (parent, parsedNode, elementDefinition) {
		...
		try {
			...
			throw Error (`Validation problem with ${this.getPosition ()}: ${cause.message}`, {cause})
	...
	blameUnexpectedTag (parsedNode, match) {
		...
		throw Error (`${this.getPosition ()}is unexpected here; should be ${expected}`)
	...
	characters (text) {
		...
		try {
			...
			throw Error (`Validation problem with the contents of ${this.getPosition ()}: ${cause.message}`, {cause})
