class XMLValidationState {
	...
	endElement (parsedNode) {
		...
		if (this.#child === null) {
			...
			const {match} = this; if (match && !match.isSatisfied) throw Error (`Unexpected ${parsedNode.src} at position ${this.getPosition () - parsedNode.src.length}, expected: ${match.allExpected (parsedNode)}`)
