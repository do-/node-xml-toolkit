class Frame {
	...
	constructor (validator, parent, parsedNode) {
		...
		try {
			...
			const elementDefinifion = parent ? parent.getChildElement (parsedNode) : xs.getSchema (parsedNode.namespaceURI).getElement (parsedNode.localName) 
			...
			throw Error (`Validation problem with ${parsedNode.src} at position ${this.validator.getPosition ()}: ${cause.message}`, {cause})
	...
	checkText (text) {
		...
		try {
			...
			throw Error (`Validation problem with the contents of <${this.parsedNode.name}...> at position ${this.validator.getPosition ()}: ${cause.message}`, {cause})
...
class XMLValidatior {
	...
	startElement (parsedNode) {
		...
		this.stack.push (new Frame (this, this.lastFrame, parsedNode))
		this.textLength = 0
