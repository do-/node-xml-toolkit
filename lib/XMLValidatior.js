...
// console.log (cause)
	...
	checkText (text) {
		...
		if (!('anyType' in this)) return

		const {_xsSimpleType} = this.anyType.node; if (!_xsSimpleType) return

// console.log ([text, _xsSimpleType.patterns])

		const result = _xsSimpleType.test (text)
...
class XMLValidatior {
	...
	characters (text) {
		...
		const {lastFrame} = this; if (!lastFrame) throw ('Character data cannot be placed outside the root element')

		lastFrame.checkText (text)
