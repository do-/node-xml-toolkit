...
const MoxyLikeJsonEncoder = function (options = {}) {
	...
	const xform = ({children, attributes}) => {
		...
		const set = (newValue, localName, namespaceURI, isMultiple = false) => {
		
			const key = getName (localName, namespaceURI)

			if (result === null) result = {}

			if (!(key in result)) return result [key] = isMultiple ? [newValue] : newValue
		...
		for (const child of children) switch (child.type) {
			...
			case END_ELEMENT:
				...
				set (xform (child), child.localName, child.namespaceURI, child [Symbol.for ('_multiple')])
