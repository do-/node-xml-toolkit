	...
	validate (xmlNode) {
		...
		if (!this.validator) switch (xmlNode.type) {
			...
			case SAXEvent.TYPES.START_ELEMENT:									
				this.validator = new XMLValidatior (this.xs, xmlNode, () => this.lex.linePos)
		...
		const {length} = chunk; if (length !== 0) {
			...
			switch (type) {
				...
				case SAXEvent.TYPES.END_ELEMENT:
					...
					if (element === null)       return callback (new Error (this.lex.linePos.format ('Unbalanced end element')))

					if (e.name != element.name) return callback (new Error (this.lex.linePos.format ('Unmatched end element, </${%s}> expected', this.element.name)))
