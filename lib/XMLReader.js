		...
		const {length} = chunk; if (length !== 0) {
			...
			switch (type) {
				...
				case SAXEvent.TYPES.END_ELEMENT:
					...
					if (element === null) return callback (new Error (`${this.lex.linePos}Unbalanced end element`))

					if (e.name != element.name) return callback (new Error (`${this.lex.linePos}Unmatched end element, </${element.name}> expected`))
