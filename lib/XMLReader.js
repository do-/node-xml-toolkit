		...
		const {length} = chunk; if (length !== 0) {
			...
			switch (type) {
				...
				case SAXEvent.TYPES.END_ELEMENT:
					...
					if (element === null)       return callback (new Error (this.lex.linePos.format ('XML-00002')))

					if (e.name != element.name) return callback (new Error (this.lex.linePos.format ('XML-00003', [this.element.name])))
