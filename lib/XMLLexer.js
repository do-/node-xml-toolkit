...
const XMLLexer = class extends Transform {
	...
	sliceTo (to) {
		...
		const s = this.body.slice (this.start, ++ to)
			...
			switch (this.state) {
				...
				case ST_TEXT:
				
					{
						...
						const pos = this.body.indexOfLt (this.start); switch (pos) {
				...
				case ST_LT: 
				
					{
						...
						const pos = this.start + 1; if (pos >= this.body.size ()) return null

						switch (this.body.charCodeAt (pos)) {
				...
				case ST_LT_X:
				
					{
						...
						const pos = this.start + 2; if (pos >= this.body.size ()) return null
				...
				case ST_TAG:
					...
					let pos = this.start; while (true) {
						...
						pos = this.body.indexOfGt (pos); if (pos === -1) return
				...
				case ST_TAG_X:
					...
					for (let pos = start + 1; pos < this.body.size (); pos ++) {
